...
import { Card, Alert, Typography, Button, ButtonGroup, Tooltip, Box, Container, Stack } from '@mui/material'
import { FileDownload, PictureAsPdf, TableChart, Code } from '@mui/icons-material'
...
const Page = () => {
  ...
  const handleExport = (format) => {
    const data = reportDataApi.data
    if (!data || !Array.isArray(data) || data.length === 0) {
      return
    }

    const filename = `m365-${source}-${report}-${currentTenant}-${new Date().toISOString().split('T')[0]}`
    const title = `${prettifyReportName(report)} - ${currentTenant}`

    switch (format) {
      case 'csv':
        ExportUtils.csv(data, filename)
        break
      case 'excel':
        ExportUtils.excel(data, filename)
        break
      case 'json':
        ExportUtils.json(data, filename)
        break
      case 'pdf':
        ExportUtils.pdf(data, filename, { title })
        break
      default:
        ExportUtils.csv(data, filename)
    }
  ...
  const hasData = reportDataApi.data && Array.isArray(reportDataApi.data) && reportDataApi.data.length > 0

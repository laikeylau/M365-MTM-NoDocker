    ...
    try {
        ...
        foreach ($tenant in $tenants) {
            try {
                ...
                $ApplicationList = New-GraphGetRequest -Uri $baseuri -tenantid $tenant -AsApp $true | Where-Object { $_.DisplayName -eq $AppConfig.Applicationname -and ($_.'@odata.type' -eq '#microsoft.graph.win32LobApp' -or $_.'@odata.type' -eq '#microsoft.graph.winGetApp') }

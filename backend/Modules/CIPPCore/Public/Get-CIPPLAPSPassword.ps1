...
function Get-CIPPLapsPassword {
    ...
    try {
        $GraphRequest = (New-GraphGetRequest -NoAuthCheck $true -uri "https://graph.microsoft.com/beta/directory/deviceLocalCredentials/$($device)?`$select=credentials" -tenantid $TenantFilter -AsApp $true).credentials | Select-Object -First 1 | ForEach-Object {

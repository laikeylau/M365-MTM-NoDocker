        ...
        try {
            ...
            $BatchResponses = $BatchRequests | ForEach-Object -Parallel {
                ...
                try {
                    ...
                    Import-Module AzTablesShim -Force -ErrorAction SilentlyContinue
...
function Receive-CIPPTimerTrigger {
    ...
    .FUNCTIONALITY
        Entrypoint
    #>
    param($Timer, [object[]]$DueFunctions)

    $UtcNow = (Get-Date).ToUniversalTime()
    $Functions = if ($DueFunctions) { @($DueFunctions) } else { Get-CIPPTimerFunctions }
    ...
    foreach ($Function in $Functions) {
        ...
        if (-not $FunctionStatus) {
            $FunctionStatus = [PSCustomObject]@{ PartitionKey = 'CIPPTimer'; RowKey = $Function.Id }
        }

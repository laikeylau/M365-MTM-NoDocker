Function Invoke-ListUsers {
    ...
    $GraphRequest = if ($TenantFilter -ne 'AllTenants') {
        New-GraphGetRequest -uri "https://graph.microsoft.com/beta/users/$($userid)?`$top=999&`$filter=$GraphFilter&`$count=true&`$expand=manager(`$select=id,userPrincipalName,displayName)" -tenantid $TenantFilter -AsApp $true -ComplexFilter | ForEach-Object {
    ...
    if ($userid -and $Request.query.IncludeLogonDetails) {
        ...
        $AuditlogsLogon = (New-ExoRequest -tenantid $TenantFilter -cmdlet 'Search-unifiedAuditLog' -cmdParams $SearchParam -AsApp | Sort-Object -Property CreationDate | Select-Object -Last 1).auditdata | ConvertFrom-Json

function Get-CIPPAlertSharepointQuota {
    ...
    try {
        ...
        $sharepointQuota = (New-GraphGetRequest -extraHeaders $extraHeaders -scope "$($SharePointInfo.AdminUrl)/.default" -tenantid $TenantFilter -uri "$($SharePointInfo.AdminUrl)/_api/StorageQuotas()?api-version=1.3.2" -AsApp $true)
    } catch {
        return
    }
    if ($sharepointQuota) {
